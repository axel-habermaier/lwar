...
//     Tuesday, 13 August 2013, 21:34:23
//
...
namespace Lwar.Scripting
{
	...
	internal static class Commands
	{
		...
		{
			StartServerCvar = new Command("start_server", "Starts up a new local server instance. If a local server is currently running, it is shut down before the new server is started.", false);
			StopServerCvar = new Command("stop_server", "Shuts down the currently running server.", false);
			ConnectCvar = new Command<IPAddress, ushort>("connect", "Connects to a game session on a remote or local server.", false, 
			...
			DisconnectCvar = new Command("disconnect", "Disconnects from the current game session.", false);
			SayCvar = new Command<string>("say", "Sends a chat message to all peers.", false, 
			...
			ToggleDebugCameraCvar = new Command("toggle_debug_camera", "Toggles between the game and the debugging camera.", false);
		...
		public static void Resolve()
		{
			...
			ResetCvar = CommandRegistry.Resolve<string>("reset");
			...
			PrintAppInfoCvar = CommandRegistry.Resolve("print_app_info");
			BindCvar = CommandRegistry.Resolve<InputTrigger, string>("bind");
			UnbindCvar = CommandRegistry.Resolve<InputTrigger>("unbind");
			UnbindAllCvar = CommandRegistry.Resolve("unbind_all");
			ListBindingsCvar = CommandRegistry.Resolve("list_bindings");
			...
			ToggleCvar = CommandRegistry.Resolve<string>("toggle");
		}
	}
}
