namespace Pegasus.Platform.Graphics
{
	...
	public sealed class IndexBuffer : Buffer
	{
		...
		private IndexBuffer(GraphicsDevice graphicsDevice, ResourceUsage usage, IntPtr indices, int size, IndexSize indexSize)
		...
		/// <param name="usage">A value describing the usage pattern of the buffer.</param>
		public static IndexBuffer Create<T>(GraphicsDevice graphicsDevice, T[] indices, ResourceUsage usage = ResourceUsage.Static)
			where T : struct
		{
			...
			using (var ptr = PinnedPointer.Create(indices))
				return new IndexBuffer(graphicsDevice, usage, ptr, Marshal.SizeOf(typeof(T)) * indices.Length, GetIndexSize<T>());
		}
		...
		/// <param name="usage">A value describing the usage pattern of the buffer.</param>
		public static IndexBuffer Create<T>(GraphicsDevice graphicsDevice, int indexCount, ResourceUsage usage = ResourceUsage.Static)
			where T : struct
		{
			Assert.ArgumentNotNull(graphicsDevice);
			Assert.ArgumentInRange(indexCount, 0, typeof(T) == typeof(uint) ? Int32.MaxValue : UInt16.MaxValue);

			return new IndexBuffer(graphicsDevice, usage, IntPtr.Zero, Marshal.SizeOf(typeof(T)) * indexCount, GetIndexSize<T>());
		}
