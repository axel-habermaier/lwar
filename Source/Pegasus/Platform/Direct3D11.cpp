...
namespace Direct3D11
{
	...
	{
		PG_ASSERT_NOT_NULL(obj);
		PG_ASSERT_NOT_NULL_OR_WHITESPACE(name);

#ifdef DEBUG
		obj->SetPrivateData(WKPDID_D3DDebugObjectName, safe_static_cast<UINT>(String::Length(name) + 1), name);
#else
		PG_UNUSED(obj);
		PG_UNUSED(name);
#endif
	...
	{
		if (FAILED(hr))
			PG_DIE("{0}", Win32::GetError(msg, static_cast<DWORD>(hr)));
	}
	...
	DXGI_FORMAT Map(IndexSize indexSize)
	{
		switch (indexSize)
		{
		case IndexSize::SixteenBit:
			return DXGI_FORMAT_R16_UINT;
		case IndexSize::ThirtyTwoBit:
