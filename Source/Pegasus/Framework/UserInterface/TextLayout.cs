namespace Pegasus.Framework.UserInterface
{
	...
	internal struct TextLayout
	{
		...
		/// <summary>
		///     The actual arranged text drawing size. Usually, the actual size is smaller than the desired size.
		...
		/// <summary>
		///     The desired drawing size of the text during the arrange phase. If the text doesn't fit, it overlaps vertically.
		/// </summary>
		private Size _desiredArrangeSize;

		/// <summary>
		///     The desired drawing size of the text during the measure phase. If the text doesn't fit, it overlaps vertically.
		/// </summary>
		private Size _desiredMeasureSize;
		...
		/// <summary>
		///     The actual measured text drawing size. Usually, the actual size is smaller than the desired size.
		///     If any words overlap, however, the actual size is bigger.
		/// </summary>
		...
		public Size Measure(Font font, string text, Size desiredSize, int lineSpacing, TextAlignment alignment, TextWrapping wrapping)
		{
			...
			if (_font == font && _text == text && _desiredMeasureSize == desiredSize && _lineSpacing == lineSpacing &&
				_alignment == alignment && _wrapping == wrapping)
				return _measuredSize;

			_font = font;
			_text = text;
			_desiredMeasureSize = desiredSize;
			...
			if (wrapping == TextWrapping.NoWrap)
				_measuredSize = new Size(font.MeasureWidth(text), font.LineHeight + lineSpacing);
			else
			{
				ComputeCharacterAreasAndLines();
				_measuredSize = ComputeActualSize();
			}
		...
		public Size Arrange(Font font, string text, Size desiredSize, int lineSpacing, TextAlignment alignment, TextWrapping wrapping)
		{
			...
			if (_font == font && _text == text && _desiredArrangeSize == desiredSize && _lineSpacing == lineSpacing &&
				_alignment == alignment && _wrapping == wrapping)
				return _arrangedSize;

			_font = font;
			_text = text;
			_desiredArrangeSize = desiredSize;
			_lineSpacing = lineSpacing;
			_alignment = alignment;
			_wrapping = wrapping;
			_lineCount = 0;
			...
			ComputeCharacterAreasAndLines();
			_arrangedSize = ComputeActualSize();
			AlignLines();
		...
		private void ComputeCharacterAreasAndLines()
		{
			...
			using (var text = TextString.Create(_text))
			{
				...
				if (_wrapping == TextWrapping.NoWrap)
				{
					...
					ComputeCharacterAreas(text, sequence, ref offset);
					line.Append(sequence, offset.X);
				}
				else
				{
					var stream = new TextTokenStream(_font, text, _desiredMeasureSize.Width);
		...
		private void AlignLines()
		{
			...
			for (var i = 0; i < _lineCount; ++i)
			{
				...
				if (_alignment == TextAlignment.Right)
					delta.X = _desiredArrangeSize.Width - _lines[i].Width;
				else if (_alignment == TextAlignment.Center)
					delta.X = (_desiredArrangeSize.Width - _lines[i].Width) / 2;
		...
		///     If any words overlap, however, the actual size is bigger.
		/// </summary>
		private Size ComputeActualSize()
		{
			var width = 0;
			...
			return new Size(width, ComputeHeight());
		}
