...
//     Tuesday, 20 August 2013, 17:10:07
//
...
namespace Pegasus.Scripting
{
	...
	internal static class Commands
	{
		...
		/// </summary>
		public static Command PrintAppInfoCvar { get; private set; }
		...
		/// </summary>
		public static Command<InputTrigger> UnbindCvar { get; private set; }

		/// <summary>
		///   Removes all command bindings.
		/// </summary>
		public static Command UnbindAllCvar { get; private set; }

		/// <summary>
		///   Lists all active bindings.
		/// </summary>
		public static Command ListBindingsCvar { get; private set; }
		...
		/// </summary>
		public static Command<string> ToggleCvar { get; private set; }
		...
		{
			add { PrintAppInfoCvar.Invoked += value; }
			remove { PrintAppInfoCvar.Invoked -= value; }
		}
		...
		{
			add { UnbindCvar.Invoked += value; }
			remove { UnbindCvar.Invoked -= value; }
		}
		...
		{
			add { UnbindAllCvar.Invoked += value; }
			remove { UnbindAllCvar.Invoked -= value; }
		}
		...
		{
			add { ListBindingsCvar.Invoked += value; }
			remove { ListBindingsCvar.Invoked -= value; }
		}
		...
		{
			add { ToggleCvar.Invoked += value; }
			remove { ToggleCvar.Invoked -= value; }
		}
		...
		{
			ExitCvar = new Command("exit", "Immediately exits the application.", false);
			HelpCvar = new Command<string>("help",
			...
			ResetCvar = new Command<string>("reset", "Resets the given cvar to its default value.", false,
											new CommandParameter("cvar", typeof(string), false, default(string),
																 "The name of the cvar that should be reset to its default value.", new NotEmptyAttribute()));
			ListCvarsCvar = new Command<string>("list_cvars", "Lists all cvars with names that match the search pattern.", false,
			...
			ListCommandsCvar = new Command<string>("list_commands", "Lists all commands with names that match the search pattern.", false,
												   new CommandParameter("pattern", typeof(string), true, "*",
																		...
																		new NotEmptyAttribute()));
			ExecuteCvar = new Command<string>("execute", "Executes the given command.", false,
											  new CommandParameter("command", typeof(string), false, default(string),
																   "The command that should be executed, including its arguments.", new NotEmptyAttribute()));
			ProcessCvar = new Command<string>("process", "Processes the commands in the given file.", false,
											  new CommandParameter("fileName", typeof(string), false, default(string),
																   "The name of the file in the application's user directory that should be processed.", new NotEmptyAttribute(),
																   new FileNameAttribute()));
			PersistCvar = new Command<string>("persist", "Saves the persistent cvars into the given file.", false,
											  new CommandParameter("fileName", typeof(string), false, default(string),
																   "The name of the file in the application's user directory the cvars should be written to.", new NotEmptyAttribute(),
																   new FileNameAttribute()));
			PrintAppInfoCvar = new Command("print_app_info", "Prints information about the application.", false);
			BindCvar = new Command<InputTrigger, string>("bind",
			...
			UnbindCvar = new Command<InputTrigger>("unbind", "Unbinds all commands currently bound to a logical input.", false,
												   new CommandParameter("trigger", typeof(InputTrigger), false, default(InputTrigger), "The trigger that should be unbound."));
			UnbindAllCvar = new Command("unbind_all", "Removes all command bindings.", false);
			ListBindingsCvar = new Command("list_bindings", "Lists all active bindings.", false);
			ShowConsoleCvar = new Command<bool>("show_console", "Shows or hides the console.", false,
												new CommandParameter("show", typeof(bool), false, default(bool), "A value of 'true' indicates that the console should be shown."));
			ReloadAssetsCvar = new Command("reload_assets", "Reloads all changed assets.", false);
			RestartGraphicsCvar = new Command("restart_graphics", "Restarts the graphics subsystem after a resolution or video mode change.", false);
			ToggleCvar = new Command<string>("toggle", "Toggles the value of a Boolean console variable.", false,
											 new CommandParameter("cvar", typeof(string), false, default(string),
																  "The name of console variable whose value should be toggled.", new NotEmptyAttribute()));
			...
			CommandRegistry.Register(PrintAppInfoCvar);
			CommandRegistry.Register(BindCvar);
			CommandRegistry.Register(UnbindCvar);
			CommandRegistry.Register(UnbindAllCvar);
			CommandRegistry.Register(ListBindingsCvar);
			...
			CommandRegistry.Register(ToggleCvar);
		}
