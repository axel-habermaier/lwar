...
pgVoid pgCreateQueryCore(pgQuery* query)
{
	...
	D3DCALL(ID3D11Device_CreateQuery(DEVICE(query), &desc, &query->ptr), "Failed to create query.");
}

pgVoid pgDestroyQueryCore(pgQuery* query)
{
	ID3D11Query_Release(query->ptr);
}

pgVoid pgBeginQueryCore(pgQuery* query)
{
	ID3D11DeviceContext_Begin(CONTEXT(query), (ID3D11Asynchronous*)query->ptr);
}

pgVoid pgEndQueryCore(pgQuery* query)
{
	ID3D11DeviceContext_End(CONTEXT(query), (ID3D11Asynchronous*)query->ptr);
}
...
pgVoid pgGetQueryDataCore(pgQuery* query, pgVoid* data, pgInt32 size)
{
	...
	case PG_TIMESTAMP_QUERY:
		hr = ID3D11DeviceContext_GetData(CONTEXT(query), (ID3D11Asynchronous*)query->ptr, data, size, 0);
		break;
	case PG_TIMESTAMP_DISJOINT_QUERY:
	{
		...
		hr = ID3D11DeviceContext_GetData(CONTEXT(query), (ID3D11Asynchronous*)query->ptr, &result, sizeof(D3D11_QUERY_DATA_TIMESTAMP_DISJOINT), 0);
...
{
	HRESULT hr = ID3D11DeviceContext_GetData(CONTEXT(query), (ID3D11Asynchronous*)query->ptr, NULL, 0, 0);
