...
namespace Pegasus.AssetsCompiler
{
    ...
    public sealed class XmlParser
    {
        ...
        {
            var children = element.Descendants(name);
            if (!children.Any())
                throw new InvalidOperationException(String.Format("Unable to find element '{0}'.", name));
            if (children.Count() > 1)
                throw new InvalidOperationException(
                    String.Format("More than one element '{0}' found. Expected only one.", name));
        ...
        public XAttribute FindAttribute(XElement element, string name)
        {
            ...
            if (attribute == null)
                throw new InvalidOperationException(String.Format("Unable to find attribute '{0}'.", name));
            return attribute;
        }
        ...
        public uint ReadAttributeUInt32(XElement element, string name)
        {
            ...
            if (!UInt32.TryParse(attribute.Value, out result))
                throw new InvalidOperationException(
                    String.Format("Unable to read value of attribute '{0}'. Expected an unsigned integer.", name));
            return result;
        }
        ...
        public int ReadAttributeInt32(XElement element, string name)
        {
            ...
            if (!Int32.TryParse(attribute.Value, out result))
                throw new InvalidOperationException(
                    String.Format("Unable to read value of attribute '{0}'. Expected a signed integer.", name));
            return result;
        }
        ...
        public ushort ReadAttributeUInt16(XElement element, string name)
        {
            ...
            if (!UInt16.TryParse(attribute.Value, out result))
                throw new InvalidOperationException(
                    String.Format("Unable to read value of attribute '{0}'. Expected an unsigned short.", name));
            return result;
        }
        ...
        public short ReadAttributeInt16(XElement element, string name)
        {
            ...
            if (!Int16.TryParse(attribute.Value, out result))
                throw new InvalidOperationException(
                    String.Format("Unable to read value of attribute '{0}'. Expected a signed short.", name));
            return result;
        }
        ...
        public string ReadAttributeString(XElement element, string name)
        {
            ...
            if (attribute.Value == null)
                throw new InvalidOperationException(String.Format("Invalid string value for attribute '{0}'.", name));
