	...
	public sealed class Cvar<T> : ICvar
	{
		...
		private bool ValidateValue(T value)
		{
			foreach (var validator in _validators)
			{
				...
				Log.Error("'{0}' could not be set to '{1}\0': {2}", Name, TypeRegistry.ToString(value, quotedContext: true), validator.ErrorMessage);
		...
		private void UpdateValue(T value)
		{
			...
			var oldValue = _value;
			_value = value;
			Log.Info("'{0}' is now '{1}\0'.", Name, TypeRegistry.ToString(value, quotedContext: true));
		...
		private void DeferredUpdate(T value)
		{
			...
			Log.Info("'{0}' will be set to '{1}\0'.", Name, TypeRegistry.ToString(value, quotedContext: true));
