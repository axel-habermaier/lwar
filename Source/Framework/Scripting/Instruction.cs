		...
		public void Execute(bool executedByUser)
		{
			...
			if (cvar != null && _parameter == null)
			{
				...
				if (cvar.UpdateMode != UpdateMode.Immediate && cvar.HasDeferredValue)
					deferred = String.Format(", pending update: '{0}\\white'", TypeRegistry.ToString(cvar.DeferredValue, quotedContext: true));

				Log.Info("'{0}' is '{1}\\white', default '{2}\\white'{3}", cvar.Name, TypeRegistry.ToString(cvar.Value, quotedContext: true),
